...
Detail for `aifullpeople-developer`'s task loop. Adapted from the exemplogs
...
`aifullpeople-pm` and `aifullpeople-tech-lead` never commit — only `developer` does.
...
For each enabled Gate in `.aifullpeople/config.yaml: gates`, resolve its command in this
order:
