...
Detail for `fullcast-developer-fix-runner`. Adapted and heavily trimmed from a
...
Same rule `fullcast-tech-lead` and `fullcast-developer` already use:
...
report why, and point back to `fullcast-tech-lead` to regenerate the three-file
...
Discover Gate commands the same way `fullcast-developer` does — check
...
red Gate, same tri-state semantics as `fullcast-developer/references/
...
it), and the Gate result. This mirrors `fullcast-developer`'s own handoff
