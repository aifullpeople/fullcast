---
name: fullcast-developer
description: |
  Developer role (persona: Miles Morales) of the fullcast framework. Implements a
  ...
  fullcast-tech-lead has produced a feature's three files. Keywords: "implement
  this feature", "fullcast developer", "run the tasks".
---
...
hands off to `fullcast-evaluator` once the full-suite Gates are green AND a human has
...
Locate the target feature's folder
(`.fullcast/<initiative_id>-<slug>/features/<id>-<name>/`) — by ID,
name, or path (the feature's `initiative_id` in `state.json` tells you which initiative
folder to look under). Require `tech-lead/design.md`, `tech-lead/tasks.md`, and
`tech-lead/contract.md` all present; abort with a clear message if any is missing
(never partially implement against an incomplete trio). Read `.fullcast/config.yaml`
...
`../fullcast-init/scripts/lock.sh acquire <feature-folder>/.lock developer`. Non-zero
...
3. **For each task in `tasks.md`, in order:**
   ...
   6. Write `<feature-folder>/developer/<task-id>.md` from
   ...
   8. Commit: `scripts/commit.sh "<message>" <code files this task touched>
      .fullcast/state.json <feature-folder>/developer/<task-id>.md`.
      `.fullcast/` is versioned in the project's git repo by default (this
      ...
      has explicitly opted the project out of versioning `.fullcast/` (rare —
...
5. **Code review (feature scope).** Once full-suite Gates are green, invoke
   `fullcast-developer-codereview` with `feature=<this feature>` (as its own
...
7. **Handoff**, only after approval: set the feature's `state.json` `stage` to
   `"validation"`. Report that `fullcast-evaluator` is next. Do not claim the feature is
...
before the first task; run `fullcast-developer-codereview` (feature scope) after
