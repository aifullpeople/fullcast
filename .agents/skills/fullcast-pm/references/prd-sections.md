...
Detail for the `requirements` stage of `fullcast-pm`. Loaded only when actually
drafting `prd.md` — adapted from the exemplogs `prd-writer` skill, translated into this
framework's vocabulary (`<initiative>/pm/prd.md` instead of a generic PRD path; content
...
rule under §2 in `fullcast-tech-lead`'s `design-and-tasks-rules.md` if unsure).
...
Note: these Section 9 criteria are the SOURCE that `fullcast-tech-lead` later turns
