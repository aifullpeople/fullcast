...
Detail for the `design` and `tasks` stages of `fullcast-tech-lead`. Loaded when
...
1. Identify the target feature from the PRD (by ID or name; ask if ambiguous). PRD is
   mandatory — if no `.fullcast/*/pm/prd.md` covers it, direct the user
   to `fullcast-pm` first.
...
Save to `.fullcast/<initiative-id>-<slug>/features/<feature-id>-<kebab-name>/tech-lead/{design.md,tasks.md,contract.md}`.
