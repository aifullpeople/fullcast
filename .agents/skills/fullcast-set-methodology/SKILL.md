---
name: fullcast-set-methodology
description: |
  Changes the DEFAULT methodology for new features in .fullcast/config.yaml. Does
...
- A feature can only be moved to `stage: done` by the methodology recorded on it. If
  today only `fullcast` exists as a methodology, this is a no-op in practice — but
...
1. Read `.fullcast/config.yaml`. Confirm the requested methodology is one this
   installation actually supports (today: `fullcast` only).
