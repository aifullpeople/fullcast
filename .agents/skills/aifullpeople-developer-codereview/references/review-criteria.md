...
files as the concrete standard, instead of restating general knowledge those files
already cover.
...
|---|---|---|
| **Design/Structure** | Does the diff match `design.md`'s Component Overview (right files, right responsibilities)? Is the abstraction level appropriate — no god object, no premature abstraction? | `guidelines/solid-principles.md`, `guidelines/anti-patterns.md`, `guidelines/<stack>/anti-patterns.md` |
...
| **Performance** | Obvious N+1 pattern? Unbounded loop over external data? Blocking I/O where the stack's idiom is non-blocking? | `guidelines/<stack>/*.md` idioms (e.g. Go: goroutine/error-group patterns in `guidelines/go/go.md`) |
| **Tests** | Do tests assert behavior, not implementation (guidelines/anti-patterns.md's "testing implementation instead of behavior")? Are failure paths tested, not just the happy path? | `guidelines/testing.md`, `guidelines/<stack>/testing.md` |
| **Naming** | Consistent with the codebase's own convention (check `context_project.md` first)? Meaningful, not abbreviated for no reason? | `guidelines/naming-conventions.md`, `guidelines/<stack>/naming-conventions.md` |
| **Error Handling** | Every error handled or propagated with context, never swallowed? Distinguishes expected failure from a genuine bug? | `guidelines/error-handling.md`, `guidelines/<stack>/error-handling.md` |
