---
name: fullcast-developer-codereview
description: |
  ...
  Distinct from fullcast-evaluator (behavior only, never reads source) and
  from the Gates (mechanical tools only). Use after fullcast-developer
  ...
  diff", "fullcast codereview".
---

# fullcast: Developer Code Review

Not a pipeline stage of its own — a pass `fullcast-developer` runs on itself
(same persona, Miles Morales, no new character) before handing a feature to
`fullcast-evaluator`. See `docs/architecture.md` §20 for why this exists: Gates
...
the same persona as `fullcast-developer` — a fresh subagent context is what gives
this pass real distance from the code, not a new character. When the orchestrating tool
supports isolated subagent worktrees (Claude Code's Agent tool: `isolation:
...
- Determine `stack.primary_language` (and `guidance_skill`/`guidance_source`) from
  `.fullcast/config.yaml` to resolve this stack's concrete guidance source
...
- No lock acquisition of its own — it's read-only on everything except the report file
  ...
  `fullcast-developer` already makes under that role's own lock.
...
4. **Write the report** from `assets/codereview-report-template.md` to
   `<feature-folder>/developer/codereview-feature-<ISO-ts>.md` (feature scope) or
   `.../developer/codereview-<task-id>-<ISO-ts>.md` (task scope) — `<feature-folder>` is
   `.fullcast/<initiative_id>-<slug>/features/<id>-<name>/`.
...
5. **Present, never decide.** This skill produces no verdict and blocks nothing —
   ...
   reports fold into `fullcast-developer`'s existing HiTL handoff checkpoint
...
6. **If the human wants a finding addressed now:** this is still pre-`evaluator`, so
   it's still `fullcast-developer`'s work, under `fullcast-developer`'s own
   lock — not `fullcast-developer-fix-runner`, which exists specifically for
...
`fullcast-developer-fix-runner` (that skill is for `evaluator` rejections only);
