...
Detail for `fullcast-evaluator`'s validation pass. This is "how to interpret and execute a
...
Do not start walking `contract.md` until `fullcast-developer`'s full-suite Gates
...
`<feature-folder>/evaluator/eval-report-<ISO-timestamp>.md` — one row per
item (✓/✗ + reason), regardless of the final verdict. This is what makes the walk
readable outside this run's own context — `fullcast-developer-fix-runner`
...
3. Do NOT reopen already-`done` tasks and do NOT invent new task entries yourself —
   ...
   `fullcast-developer-fix-runner` (architecture.md §19) exists to preserve.
...
Then:
1. Write all three files to `<feature-folder>/evaluator/`.
2. Update `state.json`: `features[].stage` → `"done"`; `features[].real_difficulty` →
   ...
   `tokens.md`.
3. Update `.fullcast/report.md` (project-level rollup) — add or refresh this
...
5. **Commit the approval.** `fullcast-evaluator` has no `scripts/` of its own — reuse
   `fullcast-developer`'s `commit.sh` (they're always installed side by side;
   found needed the first time this framework was run end-to-end):
   ```
   ../fullcast-developer/scripts/commit.sh "chore(<id>): Evaluator approval — feature done" \
     .fullcast/state.json .fullcast/report.md \
     <feature-folder>/evaluator/{summary,difficulty,tokens}.md
   ```
   Same rule as every other commit in this framework: `.fullcast/` is versioned,
   never `git add -A`.
6. **Then run the initiative check** — `fullcast-evaluator/SKILL.md` Step 6.5
   (architecture.md §21): is every feature under this feature's `initiative_id` now
   `done`? If so, ask about `fullcast-changes` before reporting back.
