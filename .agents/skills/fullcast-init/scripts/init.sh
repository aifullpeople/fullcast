#!/usr/bin/env sh
# init.sh - scaffold .fullcast/ in the current project.
...
#
# context_project.md is NOT written by this script — it's model-authored from
# assets/context-template.<language>.md, filled in with the bootstrap interview's
# answers (references/bootstrap-interview.md), same pattern as design.md/brief.md.
# Run this script AFTER writing context_project.md, so the git baseline commit below
# (when there's no prior HEAD) captures both together.
#
# Usage:
...
# context_project.md is included here too if it already exists (the skill writes it
# before calling this script) — `|| true` on the add tolerates it not existing yet
# for a direct manual run of this script.
...
echo "Initialized .fullcast/ (language=$LANGUAGE, stack=$STACK)"
